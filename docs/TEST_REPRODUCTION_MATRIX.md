...
| :---: | :--- | :--- | :---: | :--- | :--- | :--- |
| **1** | **SQuAD Knowledge Retention** | Sec 5.3 / Fig 3 | ✅ **Executed** | **Paper:** 100 samples, ans $\le 3$ tokens, GPT-4o-mini filtered, distractors up to 160k tokens.<br>**Local:** 100 samples, ans $\le 3$ tokens, 10 distractors (`nuc=10`). | [figures/squad.pdf](file:///C:/usp/MemoryLLM/arXiv-2502.00592v2/figures/squad.pdf) | [test_qa_memory.py](file:///C:/usp/MemoryLLM/test_qa_memory.py)<br>*(Ready as-is)* |
| **2** | **NaturalQA Knowledge Retention** | Sec 5.3 / App B.1 / Fig 4 | ✅ **Executed** | **Paper:** 100 samples, ans $\le 4$ tokens, GPT-4o-mini filtered, distractors up to 160k tokens.<br>**Local:** 100 samples, ans $\le 4$ tokens, 10 distractors (`nuc=10`). | [figures/nqa.pdf](file:///C:/usp/MemoryLLM/arXiv-2502.00592v2/figures/nqa.pdf) | [test_qa_memory.py](file:///C:/usp/MemoryLLM/test_qa_memory.py)<br>*(Ready as-is)* |
...
1. **Attention Backend:** The local execution runs under PyTorch native SDPA fallback (`LlamaSdpaAttention`), whereas the paper was run using custom `FlashAttention-2` CUDA kernels.
2. **Distractor Horizon:** Local tests verified retention up to 10 distractor steps (~4,000 tokens of distraction). The paper evaluated both 10 steps and up to 200 chunks (~160k tokens) to show extreme long-term decay plateaus.
